...
wedgeSequence :: MonadLogic m => [m a] -> m [a]
wedgeSequence [] = pure []
wedgeSequence (m:ms) = m >>*- (flip fmap (wedgeSequence ms) . (\a as -> (a:as))) -- m >>*- (\a -> wedgeSequence ms >>= \as -> pure (a:as))
{-# INLINE wedgeSequence #-}


alternateMany :: MonadLogic m => [a] -> m a
alternateMany = foldr interleave empty . map pure
{-# INLINE alternateMany #-}
